...
rows=(
  "giddy|.rate_limits.five_hour.used_percentage=5"
  "happy|."
  "neutral|.rate_limits.five_hour.used_percentage=50"
  "bored|.rate_limits.five_hour.used_percentage=65"
  "sleepy|.cost.total_duration_ms=18000000"
  ...
  "boss|."
  "alarmed|.rate_limits.five_hour.used_percentage=80"
  "grim|.rate_limits.seven_day.used_percentage=93"
  "crying|.rate_limits.five_hour.used_percentage=95"
  "almost free|.rate_limits.five_hour.used_percentage=92 | .rate_limits.five_hour.resets_at=(now+600)"
...
while :; do
  ...
  for i in "${!rows[@]}"; do
    ...
    line=$(jq -nc --arg sid gallery-$i --arg t "$d/transcript" "{session_id:\$sid, transcript_path:\$t,
      ...
      | sed $'s/^[^\e]*//; s/\(\e\\[0m\\).*/\\1/')  # keep just the coloured face
    out+=$(printf '  \e[2m%-12s\e[0m %s\e[K' "$name" "$line")$'\n'
  done
